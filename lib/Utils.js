...
_axios["default"].interceptors.response.use(function (response) {
  Log.info("Response headers ".concat(prettyJson(response.headers))); // if (
  ...
  Log.error("".concat(error === null || error === void 0 ? void 0 : error.message));
...
function getApiResources(url, responseType, auth, params, configuration, options) {
  ...
  var transformResponse = function transformResponse(data, headers) {
    if (filter) {
      ...
      if ((_headers$contentType = (_headers$contentType2 = headers['content-type']) === null || _headers$contentType2 === void 0 ? void 0 : _headers$contentType2.split(';')[0]) !== null && _headers$contentType !== void 0 ? _headers$contentType : null === CONTENT_TYPES.JSON) {
        var tempData = JSON.parse(data);
        return _objectSpread({}, tempData, {
          resources: applyFilter.apply(void 0, [tempData.resources].concat(_toConsumableArray(parseFilter(filter))))
        });
      }
    }
...
function getSchema(resourceType, params, options, callback) {
  return function (state) {
    ...
    var url = buildUrl("/schemas/".concat(resourceType), hostUrl, apiVersion, useApiVersion);
    ...
    return _axios["default"].request({
      ...
      Log.info(composeSuccessMessage('getSchema', resourceType, queryParams, options, callback));
...
function getData(resourceType, params, options, callback) {
  return function (state) {
    ...
    var queryParams = (0, _languageCommon.expandReferences)(params)(state);
    var url = buildUrl(getData, resourceType, state.configuration, options);
    logApiVersion(state.configuration, options);
    logWaitingForServer(url, queryParams);
    warnExpectLargeResult(resourceType, url);
    return _axios["default"].request({
      ...
      Log.info(composeSuccessMessage(getData, resourceType, queryParams, options, callback));
...
function getMetadata(resources, params, options, callback) {
  return function (state) {
    ...
    var url = buildUrl(getMetadata, resources, state.configuration, options);
    logApiVersion(state.configuration, options);
    logWaitingForServer(url, queryParams);
    ...
    return _axios["default"].request({
      ...
      Log.info(composeSuccessMessage(getMetadata, resources, queryParams, options, callback));
      if (callback) return callback(composeNextState(state, result));
      return composeNextState(state, result);
    });
  };
...
function postData(resourceType, data, params, options, callback) {
  return function (state) {
    ...
    var url = buildUrl(postData, resourceType, state.configuration, options);
    logApiVersion(state.configuration, options);
    logWaitingForServer(url, queryParams);
    warnExpectLargeResult(resourceType, url);
    return _axios["default"].request({
...
function composeSuccessMessage(operation, responseType, params, options, callback) {
  return "".concat(operation, "('").concat(responseType !== null && responseType !== void 0 ? responseType : '', "', ").concat(JSON.stringify(params !== null && params !== void 0 ? params : null), ", ").concat(JSON.stringify(options !== null && options !== void 0 ? options : ''), ", ").concat((0, _lodash.isEmpty)(callback !== null && callback !== void 0 ? callback : '-unused-') ? '(state)=>{}' : callback, ") succeeded. The body of this result will be available in state.data or in your callback.");
}
/**
