...
function upsertTEI(uniqueAttributeId, data, options) {
  return function (state) {
    ...
    var url = resolveUrl(hostUrl + '/', "api/trackedEntityInstances");
...
function upsert(resourceType, uniqueAttribute, updateCondition, data, params, options, callback) {
  return function (state) {
    var _options$apiVersion8, _options$supportApiVe8, _CONTENT_TYPES$respon5;
    ...
    return Promise.all([getResouceName(), findRecordsWithValueOnAttribute()]).then(function (_ref) {
      var _ref2 = _slicedToArray(_ref, 2),
          resourceName = _ref2[0],
          recordsWithValue = _ref2[1];

      var recordsWithValueCount = recordsWithValue.data[resourceType].length;

      if (recordsWithValueCount > 1) {
        throw new Error("Cannot upsert on Non-unique attribute. The operation found more than one records with the same value of ".concat(attributeValue, " for ").concat(attributeId));
        ...
        //   );
        // }
        _Utils.Log.info("Attribute has unique values. Proceeding to replace or merge ...");
        ...
        return _axios["default"].request({
          ...
          _Utils.Log.info("Upsert succeeded. Updated ".concat(resourceName, ": ").concat(updateUrl, ".\nSummary:\n").concat((0, _Utils.prettyJson)(result.data)));

          if (callback) return callback((0, _languageCommon.composeNextState)(state, result.data));
          return (0, _languageCommon.composeNextState)(state, result.data);
        });
      } else if (recordsWithValueCount === 0) {
        _Utils.Log.info("Existing record not found, proceeding to CREATE(POST) ...");
        ...
        return _axios["default"].request({
          ...
          _Utils.Log.info("Upsert succeeded. Created ".concat(resourceName, ": ").concat(result.data.response.importSummaries[0].href, "\nSummary:\n").concat((0, _Utils.prettyJson)(result.data)));
