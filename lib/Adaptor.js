...
_axios["default"].interceptors.response.use(function (response) {
  ...
  // Cleanup sensitive info
  _utils.Log.error("".concat(error.message));
...
function applyFilter(arrObject, filterTokens) {
  if (filterTokens) {
    try {
      ...
      _utils.Log.warn("Returned unfiltered data. Failed to apply custom filter(".concat((0, _utils.prettyJson)({
  ...
  _utils.Log.info("No filters applied, returned all records on this resource.");
...
function getResources(params, options, callback) {
  return function (state) {
    ...
    _utils.Log.info("url ".concat(url));
    ...
    return _axios["default"].request({
      ...
      _utils.Log.info("Request filter: ".concat(filter !== null && filter !== void 0 ? filter : {}));

      _utils.Log.info('getResources succeeded.The body of this result will be available in state.data or in your callback.');
...
function getSchema(resourceType, params, options, callback) {
  return function (state) {
    ...
    var path = supportApiVersion === true ? "".concat(apiVersion !== null && apiVersion !== void 0 ? apiVersion : 'api_version_missing', "/schemas/").concat(resourceType) : "/schemas/".concat(resourceType);
    var url = hostUrl + '/api/' + path;

    _utils.Log.info("url ".concat(url));

    if ((0, _lodash.isEmpty)(resourceType)) _utils.Log.warn("This endpoint may return a large collection of schemas, since 'resourceType' is not specified. We recommend you specify 'resourceType' or use 'filter' parameter to limit the content of the result.");

    _utils.Log.info("params ".concat((0, _utils.prettyJson)(params)));
