...
function discover(operation, resourceType) {
  return function (state) {
    return _axios["default"].get('https://dhis2.github.io/dhis2-api-specification/spec/metadata_openapi.json').then(function (result) {
      // console.log(
      //   prettyJson(
      //     result.data.paths['/analyticsTableHooks']['get']['parameters']
      //   )
      // );
      console.table(result.data.components.parameters, ['in', 'description', 'schema', 'required']); // console.log(prettyJson(result.data.components.schema));
