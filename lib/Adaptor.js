    ...
    return Promise.all([getResouceName(), findRecordsWithValueOnAttribute()]).then(function (_ref3) {
      ...
      if (recordsWithValueCount > 1) {
        ...
        return _axios["default"].request({
          ...
          _Utils.Log.info("Upsert succeeded. Updated ".concat(resourceName, ": ").concat(_Utils.COLORS.FgGreen).concat(updateUrl).concat(_Utils.ESCAPE, ".\nSummary:\n").concat((0, _Utils.prettyJson)(result.data)));

          if (callback) return callback((0, _languageCommon.composeNextState)(state, result.data));
          return (0, _languageCommon.composeNextState)(state, result.data);
        });
      } else if (recordsWithValueCount === 0) {
        _Utils.Log.info("Existing record not found, proceeding to ".concat(_Utils.COLORS.FgGreen, "CREATE(POST)").concat(_Utils.ESCAPE, " ..."));
        ...
        return _axios["default"].request({
          ...
          _Utils.Log.info("Upsert succeeded. Created ".concat(resourceName, ": ").concat(_Utils.COLORS.FgGreen).concat(result.data.response.importSummaries[0].href).concat(_Utils.ESCAPE, "\nSummary:\n").concat((0, _Utils.prettyJson)(result.data)));
