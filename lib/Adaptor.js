...
function applyFilter(arrObject, filterTokens) {
  try {
    ...
    console.log("Returned unfiltered data. Failed to apply custom filter(".concat(JSON.stringify({
      ...
      operator: (_filterTokens$ = filterTokens[1]) !== null && _filterTokens$ !== void 0 ? _filterTokens$ : null,
...
function parseFilter(filterExpression) {
  var args = filterExpression.split(':');
  args[1] = dhis2OperatorMap[args[1]];
  return args;
} // axios interceptor to custom filter resources
...
function getSchema(resourceType, params, options, callback) {
  return function (state) {
    ...
    return _axios["default"].request({
      ...
      console.log("getSchema('".concat(resourceType, "') succeeded. The body of this result will be available in state.data or in your callback"));
