...
function upsertTEI(uniqueAttributeId, data, options, updateCondition, callback) {
  return function (state) {
  ...
  return function (state) {
    return _axios["default"].get('https://dhis2.github.io/dhis2-api-specification/spec/metadata_openapi.json', {
      transformResponse: [function (data, headers) {
        ...
        var filteredData = tempData.paths[resourceType][operation];
        console.log("filtered paths ".concat(filteredData));
        return _objectSpread({}, tempData, {
          components: _objectSpread({}, tempData.components, {
            parameters: Object.entries(tempData.components.parameters).reduce(function (acc, currentvalue) {
              var index;
              if ((0, _lodash.indexOf)(currentvalue[1].description, ',') === -1 && (0, _lodash.indexOf)(currentvalue[1].description, '.') > -1) index = (0, _lodash.indexOf)(currentvalue[1].description, '.');else if ((0, _lodash.indexOf)(currentvalue[1].description, ',') > -1 && (0, _lodash.indexOf)(currentvalue[1].description, '.') > -1) {
                index = (0, _lodash.indexOf)(currentvalue[1].description, '.') < (0, _lodash.indexOf)(currentvalue[1].description, ',') ? (0, _lodash.indexOf)(currentvalue[1].description, '.') : (0, _lodash.indexOf)(currentvalue[1].description, ',');
              } else {
                index = currentvalue[1].description.length;
              }
              currentvalue[1].description = currentvalue[1].description.substring(0, index);

              if (currentvalue[1].schema['$ref']) {
                var schemaRefIndex = currentvalue[1].schema['$ref'].lastIndexOf('/') + 1;
                var schemaRef = currentvalue[1].schema['$ref'].slice(schemaRefIndex);
                currentvalue[1].schema = tempData.components.schemas[schemaRef];
              }

              currentvalue[1].schema = JSON.stringify(currentvalue[1].schema);
      ...
      _Utils.Log.info("\t===========================================================================\n\tQuery Parameters for ".concat(_Utils.COLORS.FgGreen).concat(operation).concat(_Utils.ESCAPE, " on ").concat(_Utils.COLORS.FgGreen).concat(resourceType).concat(_Utils.ESCAPE, "\n\t==========================================================================="));

      console.table(result.data.components.parameters, ['in', 'required', 'description']);
